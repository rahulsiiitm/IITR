...
### Added
- **Hybrid Retrieval (BM25 + FAISS)**: Integrated BM25 sparse search alongside FAISS dense search, fused via **Reciprocal Rank Fusion (RRF)** to optimize exact match queries (like clause codes and acronyms).
...
### Changed
- **Spell Tolerance & Normalization**: Implemented equivalent checks for British/American spellings (e.g. defense vs. defence) and numbers (e.g. "three" vs "3") in the test validator.
...
### Added
- **RAG MVP Prototype**: Initial implementation of the offline PhD Regulations chat assistant.
- **Index Builder**: Built the parsing and FAISS indexing pipeline using `BAAI/bge-small-en-v1.5` embeddings.
- **FastAPI Backend**: Built endpoints to query the assistant and check service health.
- **Vanilla Web UI**: Designed a glassmorphism responsive frontend chat interface using Plus Jakarta Sans and local styling tokens.
